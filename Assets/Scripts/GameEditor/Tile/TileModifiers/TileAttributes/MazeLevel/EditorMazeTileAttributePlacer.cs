...
public class EditorMazeTileAttributePlacer : MazeTileAttributePlacer<EditorMazeTile>
{
    ...
    public override void PlaceFerryRoute(string ferryRouteId = "", List<Tile> ferryRoutePointTiles = null, int dockingStartDirection = -1)
    {
        ...
        ferryRoute.Initialise(ferryRouteId, dockingStartDirection);

        ferryRoute.AddRouteLineRenderer();

        if (ferryRoutePointTiles == null)
        {
            ferryRoute.AddFerryRoutePointInEditor(_tile);
        }
        else
        {
            for (int i = 0; i < ferryRoutePointTiles.Count; i++)
            {
                ferryRoute.AddFerryRoutePointInEditor(ferryRoutePointTiles[i]);
            }
        }
