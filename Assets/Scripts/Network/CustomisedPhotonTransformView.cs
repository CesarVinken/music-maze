...
public class CustomisedPhotonTransformView : MonoBehaviour, IPunObservable
{
    ...
    public void Update()
    {
        if (!this.m_PhotonView.IsMine)
        {
            ...
            if (GameManager.Instance.GameType == GameType.Multiplayer && TeleportEnabled == true && Vector3.Distance(transform.position, this.m_NetworkPosition) > TeleportIfDistanceGreaterThan)
            {
